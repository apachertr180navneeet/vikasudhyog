...
<section class="view-section active" id="view-master-company-create">
    ...
    <form action="{{ route('admin.masters.company.store') }}" method="POST" id="company-create-form">
        @csrf
                        ...
                        <div class="form-row-3">
                            <div class="form-group">
                                <label class="form-label" style="display: flex; justify-content: space-between; align-items: center;">
                                    ...
                                    <span class="label-hint" id="code-status-badge" style="font-size: 0.72rem; color: var(--primary); font-weight: 600;">Auto Unique</span>
                                </label>
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-icon-wrap">
                                    ...
                                    <select name="status" id="field-status" class="form-control" required>
                                        <option value="active" {{ old('status', 'active') === 'active' ? 'selected' : '' }}>Active Operating</option>
                                        <option value="inactive" {{ old('status') === 'inactive' ? 'selected' : '' }}>Inactive / Dormant</option>
                                    </select>
                                </div>
                            </div>
            ...
            <div class="form-sidebar-col">
                ...
                <div class="preview-company-card" style="margin-bottom: 1.5rem;">
                    <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 1rem;">
                        ...
                        <span class="preview-badge-pill default" id="preview-default-pill" style="display: none;">
                    ...
                    <div id="preview-tagline" style="font-size: 0.8rem; opacity: 0.85; margin-bottom: 1.25rem; font-style: italic; line-height: 1.35;">
                        Herbal manufacturing &amp; trading division
                    </div>
                    ...
                    <div style="border-top: 1px solid rgba(255, 255, 255, 0.15); padding-top: 0.85rem; display: flex; flex-direction: column; gap: 0.5rem; font-size: 0.8rem;">
                        <div style="display: flex; justify-content: space-between; opacity: 0.9;">
                            <span>Code:</span>
                            <strong id="preview-code">VU-NEW</strong>
                        </div>
                        <div style="display: flex; justify-content: space-between; opacity: 0.9;">
                            <span>GSTIN:</span>
                            <strong id="preview-gstin">Not Set</strong>
                        </div>
                        <div style="display: flex; justify-content: space-between; opacity: 0.9;">
                            <span>Location:</span>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        let isManualCode = {{ old('code') ? 'true' : 'false' }};
        let codeTimer = null;
        ...
        function fetchGeneratedCode(name) {
            ...
            fetch(`{{ route('admin.masters.company.generate-code') }}?name=${encodeURIComponent(name)}`)
                .then(res => res.json())
                .then(data => {
                    if (data.success && data.code) {
                        if (!isManualCode || !codeInput.value.trim()) {
                            codeInput.value = data.code;
                            updatePreview();
                        }
                    }
                    if (codeBadge) {
                        codeBadge.textContent = isManualCode ? 'Custom Code' : 'Auto Unique';
                    }
                })
                .catch(() => {
                    if (codeBadge) {
        ...
        if (codeInput) {
            codeInput.addEventListener('input', () => {
                ...
                isManualCode = codeInput.value.trim().length > 0;
                if (codeBadge) {
                    codeBadge.textContent = isManualCode ? 'Custom Code' : 'Auto Unique';
                }
            });
